...
//  pmu_set_pin_dir(GPIO_PORT_D,BIT(4), GPIO_DIR_OUT);
//  pmu_set_gpio_value(GPIO_PORT_D, BIT(4), 1);	
  ...
  system_set_port_mux(GPIO_PORT_A,GPIO_BIT_4,PORTD4_FUNC_D4);
...
{
	uart_putc_noint_no_wait(UART1, 's'); //进入睡眠模式
...
__attribute__((section("ram_code"))) void user_entry_after_sleep_imp(void)
{
	...
	uart_putc_noint_no_wait(UART1, 'w'); //唤醒操作
	pmu_set_led2_value(0); 	//打开led	
