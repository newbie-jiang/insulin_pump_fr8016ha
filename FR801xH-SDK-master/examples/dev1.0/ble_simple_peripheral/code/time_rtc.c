...
{
    if (year != NULL) *year = clock_env.year;
    if (month != NULL) *month = clock_env.month;
    if (day != NULL) *day = clock_env.day;

    co_printf("Current Date: %04d-%02d-%02d\r\n", clock_env.year, clock_env.month, clock_env.day);
}
